...
_version = "1.0.0"
_configuration_file = "config.yml"
_username = "oph-test"
_password = "abcd"
...
class oph_esgf_subset(Process):
    ...
    def _handler(self, request, response):
        ...
        setting_server = cfg['setting_server']
        setting_port = cfg['setting_port']
        setting_path = cfg['setting_path']

        LOGGER.debug("Execute the job")

        cube.Cube.setclient(username=_username, password=_password, server=setting_server, port=setting_port)
        ...
        out_name = uuid.uuid4()
        cube1.exportnc2(output_path=setting_path,output_name=out_name)
        out_link = setting_path + '/' + out_name
        cube1.delete()
...
class oph_esgf_max(Process):
    ...
    def _handler(self, request, response):
        ...
        out_name = uuid.uuid4()
        cube2.exportnc2(output_path=setting_path,output_name=out_name)
        out_link = setting_path + '/' + out_name
        cube1.delete()
        cube2.delete()
